...
public class CourseMain {
	public static String fileName = "C:\\Users\\odi07\\eclipse-workspace\\EB_HW2\\src\\Courses.txt";
		...
		while (!done) {
			...
			for (int i = 0; i < eventQueue.getSize(); i++) {
				...
				case RegisterCourses:
					printLogEvent("Get", event);
					Thread.sleep(100);
					eventBus.sendEvent(new Event(EventId.ClientOutput, RegisterCourses(coursesList, event.getMessage())));
					break;
				case DeleteCourses:
					printLogEvent("Get", event);
					Thread.sleep(100);
	...
	private static String RegisterCourses(CourseComponent coursesList, String message) {
