...
public class RMIEventBusImpl extends UnicastRemoteObject implements RMIEventBus { //통로 역할을 함
	...
	synchronized public long register() throws RemoteException { //등록 = EventQueue에 넣어준다
	...
	synchronized public void unRegister(long id) throws RemoteException { //해지 = EventQueue에서 지움
	...
	synchronized public void sendEvent(Event sentEvent) throws RemoteException {//이벤트를 던진다= EventQueue에 이벤트를 넣는
		EventQueue eventQueue;
		for ( int i = 0; i < eventQueueList.size(); i++ ) {
			eventQueue = eventQueueList.get(i);
	...
	synchronized public EventQueue getEventQueue(long id) throws RemoteException { //EventQueue를 전체적으로 달라
